...
-}
type Arity tyname name uni ann = [ParamKind tyname name uni ann]
...
data ParamKind tyname name uni ann =
    TermParam ann name (Type tyname uni ann) | TypeParam ann tyname (Kind ann)
    deriving stock (Show)

instance (Show tyname, Show name, Show ann, GShow uni) =>
    Pretty (ParamKind tyname name uni ann) where
  pretty = viaShow
